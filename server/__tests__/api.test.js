...
describe('mixed-content', () => {
  ...
  it('Check /top-government-websites-with-mixed-content', async () => {
    ...
    expect(res.body.description).toBe('Top governmental websites with most'+
  ...
  it('Check /top-countries-with-more-government-websites-with-mixed-content',
    ...
    '/top-countries-with-more-government-websites-with-mixed-content');
        ...
        expect(res.body.description).toBe('Countries with more government'+
        ' websites that have mixed content');
        expect(res.body.result).toBeInstanceOf(Array);
  ...
  it('Check /mixed-content-percentage-histogram', async () => {
    const res = await request(app).get('/api/mixed-content/'+
    '/mixed-content-percentage-histogram');

    expect(res.body.description).toBe('Histogram of the percentage of mixed'+
    ' content of all websites.');
    expect(res.body.result).toBeInstanceOf(Array);
    expect(res.body.suggestedVisualizations).toContain('Bar chart');
  });
  ...
  it(
      'Check with more datapoints than they exist /https-percentage-pages',
      async () => {
        const res = await request(app).get(
